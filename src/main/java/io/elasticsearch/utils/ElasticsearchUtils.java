 ...
 * Author:		lichao
 * Version:		1.0  
 * Create at:	2018年5月24日 下午4:15:05  
 *  
 * 修改历史:
 * 日期    作者    版本  修改描述
 * ------------------------------------------------------------------
...
public class ElasticsearchUtils {
	...
	public static void createIndex(String index) {
		...
		LOGGER.info("执行建立成功？" + createIndexRequestBuilder.get());
	 ...
	 * @return
	 */
	public static boolean createIndex2(String index) {
		if (!isIndexExist(index)) {
			LOGGER.info("Index2 is not exits!");
		}
		CreateIndexResponse indexresponse = client.admin().indices().prepareCreate(index).setSettings(createSetting())
				.execute().actionGet();
